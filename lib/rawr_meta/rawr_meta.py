...
sys.path.append('./lib')
sys.path.append('./lib/rawr_meta/lib')
import os
import OleFileIO_PL
from docx import *
...
class Meta_Parser(object):
	def __init__(self):
		...
		try:
			...
			self.CONTENT_REGEXES = [
				 ("internal_ips",     "((?:127.0.0.1|(?:10\.(?:25[0-4]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[1-9])|192\.168|172\." +
								      "(?:1[6-9]|2[0-9]|3[0-1]))(?:\.(?:25[0-4]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[1-9])){2}))"),
				 ("email_addresses",  "([a-zA-Z0-9._+-]{2,}@[a-zA-Z0-9.-]{1,}\.(?:[A-Z]{2,4}|[a-z]{2,4}))"),
				 ("phone_numbers",    "((?:(?:[\(\[][2-9]{1}[0-9]{2}[\)\]]|[2-9]{1}[0-9]{2})[\-\s]{1}){0,1}[2-9]{3}[\-]{1}[0-9]{4})[^\-]"),
				 ("users",            "(?:[a-zA-Z]:[\\\\]{1,}[U|u]sers|[D|d]ocuments and [S|s]ettings)[\\\\]{1,}(.*?)[\\\\]{1,}"),
				 ("urls",             "(http[s]?://[0-9a-zA-Z.@:&+-]+(?:[/][0-9a-zA-Z-&\?]+)+(?:[/]|[.][0-9a-z][0-9a-z-]{0,2}|[.][0-9A-Z][0-9A-Z-]{0,2}]))"),
				 ("locations",        "[^http:](?:[a-zA-Z]:[\\\\]{1,}|file:[\\\\]{1,}|[\\\\]{1,})((?:[\\\\]{1,}[a-z.A-Z0-9]{1,30}){2,}[/|\\\\]?)")#,
				 #("sql_statements", """(ALTER|CREATE|DELETE|DROP|EXEC(?:UTE)?|INSERT(?:INTO)?|MERGE|SELECT|UPDATE|UNION(?:ALL)?[^;\"\'])"""),
		    ...
		    for item in re.findall("<<(.*?/CreationDate.*?)>>", fdat)[0].split('/')[1:]:
		        ...
		        if not (prop in self.PDF_IGNORED or len(prop) < 3):
		            if prop in ('Creator', 'Producer'):
		                ...
		                props = self.addto(props, 'dates', "<small><b>" + prop + "</b>: " + dt.strftime("%m-%d-%Y %H:%M:%S") + '</small>')
		            ...
		            else: props = self.addto(props, prop, val)
		    ...
		    for a in arr:
		        try:
		            if a[0].endswith('Date'):
		                ...
		                props = self.addto(props, 'dates', "<small><b>" + a[0] + "</b>: " + dt.strftime("%m-%d-%Y %H:%M:%S") + '</small>')
	...
	def addto(self, i1, i2, val=None):  # update a dict or combine two dicts, appending to lists instead of overwriting
		if (type(i1), type(i2), val) == (dict, dict, None):
			...
			if type(val) == dict:
				...
				try: 
					if not val in i1[i2]: i1[i2].append(val)			
				except: 
					try:
						if not val == i1[i2]: i1[i2] = [i1[i2], val]
					except: i1[i2] = val
	...
	def mod_checks(self, f):
		...
		try:
			wl = list(set( re.findall('^[a-zA-Z0-9]{3,}$', z.split()) ))
			out = {'words': wl}
				...
				if len(dat) > 0:
				    for i in list(dat):
				        if not any([t for t in ('docs.oasis-open.org', 'ns.adobe.com',
							...
							i = self.safe_string(i).replace('\\\\','\\').replace('\\\\\\\\','\\\\').replace('_x000d_', '')
							out = self.addto(out, m[0], i)
							
							if not m[0] == 'users': z = z.replace(i, '')
	...
	def msoffice_meta(self, fn):
		...
		try:
		    ...
		    if fn.split('/')[-1] in ('document.xml'):
				...
				for i in dat:
					...
					if v:
						...
						if t in ('created', 'modified', 'lastPrinted'):						
							...
							ret = self.addto(ret, 'dates', "<small><b>" + t.title() + "</b>: " + dt.strftime("%m-%d-%Y %H:%M:%S") + "</small>")
				...
				t = re.findall("lpstr>([^Worksheets]+?)</vt:lpstr>", dat)
				if len(t) > 0:
				...
				for t, v in re.findall("<(.*?)>(.*?)</", dat):
					if v and not any([i for i in ('openoffice.org', ':', '/', 'AppVersion', 'HeadingPairs',
						...
						elif not t in ('Application', 'TotalTime'):
							ret[t] = v
			
				if ret['Statistics'] == []: del ret['Statistics']
	...
	def ooo_meta(self, fn):
		...
		ret['Statistics'] = []
		try:
		    for tag, val in re.findall("meta:([a-z\-]*?)=[\"](.*?)[\"] ", dat.split('document-statistic')[1].split('/>')[0]):
		        if int(val) > 0:
	...
	def decompress(self, fn):
		...
		for item in doc.infolist():
		    ...
		    elif item.orig_filename in ('docProps/app.xml', 'docProps/core.xml') or item.orig_filename.split('/')[-1] in ('sharedStrings.xml', 'document.xml'):
		...
		for prop in (meta.SUMMARY_ATTRIBS + meta.DOCSUM_ATTRIBS):
		    ...
		    if value:
		        if prop == 'creating_application':
		        	...
		        	try: prop = prop + '</b>: ' + value.strftime("%m-%d-%Y %H:%M:%S")
		        	except: prop = prop + '</b>: Never'
		            ...
		            try: x = self.CODEPAGE_VALS[value]
		            except: x = 'Unknown: ' + str(value)
		            ret = self.addto(ret, 'Encoding', x)

		        elif prop in ('paragraphs', 'num_words', 'num_pages', 'num_chars', 'lines', 'chars_with_spaces', 'slides', 'notes'):
		            ret = self.addto(ret, 'Statistics', "%s: %s  " % (prop.replace('num_', '').replace('_', ' '), value) )

		        elif not prop in ('content_status', 'thumbnail', 'version', 'bytes', 'total_edit_time'):  # don't care

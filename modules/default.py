...
modules = {
	...
	#( "file_calls",  """show all file includes""",                                              MODTYPE_CONTENT   ),
	#( "Flash_Objects",  """/<object[^>]+application\/x-shockwave-flash[^>]+>/i""",              MODTYPE_CONTENT   ),
	#( "Flash_Objects",  """/<embed[^>]+src[\s]*=[\s]*["']?[^\s^'^"^>]+/i""",                    MODTYPE_CONTENT   ),
	...
	#( "Java_Applets",   """/<applet[^>]+code[\s]*=[\s]*["|']?([^\s^>^"^']+)[^>]*>/i""",         MODTYPE_CONTENT   ),
	#( "JQuery",         """>/jquery.js\?ver=([0-9\.]+)""",                                      MODTYPE_CONTENT   ),
	#( "IFrames",        """>/<[\s]*[i]?frame[^>]+src[\s]*=[\s]*["|']?([^>^"^'^\s]+)/i""",       MODTYPE_CONTENT   ),
